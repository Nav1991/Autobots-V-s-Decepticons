...
class Game:
    ...
    aut_win_counts = 0    #set win counts to zero for autobots
    dec_win_counts = 0    # set win counts to zero for decepticons
    battle_count = 0      # number of battles is set to zero


    def battle(self, aq, dq):
        self.battle_count += 1    # as soon as it enters here, battle starts
        # Compare the warriors and find winner
         
       
        
        
        for key in aq.keys():
            if aq[1]['Courage' + 'Strength'] < 7:  #special rule if Courage and strength combined is less than 4 or 3 i.e. 7, opponent wins
                self.dec_win_counts += 1
                print "Decpticons Win!"
            elif aq[1][key] > 3:
                self.aut_win_counts += 1
            else:
                self.dec_win_counts += 1
                
            if aq == dq:
                print "Game Ends ! Transformers destroyed"  # Game Tie, game ends
                
            if key in aq.keys() == 5:
                print "Optimus Prime is the winner!! Game Ends." 
            elif key in aq.keys() == 10:
                print "Predaking is the winner !! Game Ends"      # Optimus Prime is at number 5 and Predaking is at 10
...
if transformer1_value[0] > transformer2_value[0]:
    ...
    print (" Decepticon is Winner")
        ...
        #check for warriors for the game
        while not aq.empty() and not dq.empty()  :
            aut_warrior = aq.get()
            dec_warrior = dq.get()
        
        # who is the winner
        if self.aut_win_counts > self.dec_win_counts:
            winner = "Autobots"
        else:
            winner = "Decepticons"
        ...
        # how many battles
        print "No. of battles: {}".format(self.battle_count)
